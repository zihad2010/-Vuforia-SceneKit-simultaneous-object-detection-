...
@implementation VuforiaEAGLView {
    ...
    //Renderer
    SCNRenderer* _rendererStone;
    SCNRenderer* _rendererChip;
    
    //Camera Node
    SCNNode* _cameraNodeStone;
    SCNNode* _cameraNodeChip;
...
- (void)setupRenderer {
    ...
    _rendererChip = [SCNRenderer rendererWithContext:_context options:nil];
    _rendererChip.autoenablesDefaultLighting = YES;
    _rendererChip.playing = YES;
    
    _rendererStone = [SCNRenderer rendererWithContext:_context options:nil];
    _rendererStone.autoenablesDefaultLighting = YES;
    _rendererStone.playing = YES;
    
}

-(void)setChipsScene{
    
    sceneChip = [SCNScene sceneNamed:@"art.scnassets/circle.scn"];
    _cameraNodeChip = [SCNNode node];
    SCNCamera* camera = [SCNCamera camera];
    _cameraNodeChip.camera = camera;
    [sceneChip.rootNode addChildNode:_cameraNodeChip];
    _rendererChip.scene = sceneChip;
    _cameraNodeChip.camera.projectionTransform = _projectionTransform;
    
    _rendererChip.pointOfView = _cameraNodeChip;
}

-(void)setStonesScene{
    
    sceneStone = [SCNScene sceneNamed:@"art.scnassets/box.scn"];
    _cameraNodeStone=[SCNNode node];
    SCNCamera* camera = [SCNCamera camera];
    _cameraNodeStone.camera = camera;
    [sceneStone.rootNode addChildNode:_cameraNodeStone];
    _rendererStone.scene = sceneStone;
    _cameraNodeStone.camera.projectionTransform = _projectionTransform;
    
    _rendererStone.pointOfView = _cameraNodeStone;
...
- (void)setCameraMatrix:(Vuforia::Matrix44F)matrix with:(NSString *)string{
    ...
    if ([string isEqualToString:@"stones"]) {
        
        _cameraNodeStone.transform = inverted;
    }
    
    if ([string isEqualToString:@"chips"]) {
        
        _cameraNodeChip.transform = inverted;
    }
...
- (void)renderFrameVuforia
{
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if ([string isEqualToString:@"chips"]) {
            ...
            [_rendererChip renderAtTime: currentTime];
        }
        
        if (([string isEqualToString:@"stones"])) {
            
            [_rendererStone renderAtTime: currentTime];
        }
...
- (SCNNode*)touchedNodeWithLocationInView:(CGPoint)location {
    ...
    NSArray* resultStone = [_rendererStone hitTest:pos options:nil];
    NSArray* resultsChip = [_rendererChip hitTest:pos options:nil];
    
    if (resultStone.count != 0) {
        
        return [[resultStone firstObject] node];
    }else{
        return [[resultsChip firstObject] node];
    }
